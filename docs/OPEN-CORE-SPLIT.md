...
| **`heldar-proprietary`** | **private** | proprietary verticals (`heldar-bakery`; future `heldar-campus-*`), production bins, business/strategy docs, deploy/secrets | Proprietary |
...
(never copied): `crates/heldar-bakery`, `crates/heldar-campus-*`, internal business/strategy docs,
...
`git -C ../heldar ls-files | grep -Ei 'bakery|campus|auto\.key'` returns only benign
