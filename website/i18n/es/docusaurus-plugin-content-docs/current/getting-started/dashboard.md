...
- **Search** — consulta en lenguaje natural sobre los hechos de eventos almacenados, con una cadena de pruebas para cada respuesta. La búsqueda también incluye recuperación semántica (texto e imagen) sobre los recortes de detección almacenados, presentada como una pestaña **Semantic** (`POST /api/v1/search/semantic`), incluidas consultas acotadas a una zona («coche rojo en la zona del patio») mediante el selector de Zona por cámara de la pestaña.
