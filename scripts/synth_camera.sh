...
# A qualification run needs a PREDICTABLE bitrate: without a rate target, x264/x265 spend whatever
# the content needs and two runs of the "same" 8-camera profile push different numbers of bytes
# through the recorder, which makes the results incomparable. Capped VBV rather than CRF whenever a
# bitrate is asked for.
RATE=()
if [ "$BITRATE_KBPS" -gt 0 ]; then
  RATE=(-b:v "${BITRATE_KBPS}k" -maxrate "${BITRATE_KBPS}k" -bufsize "$((BITRATE_KBPS * 2))k")
fi
...
if [ "$ENCODER" = libx264 ]; then
  ...
  TUNE=(-x265-params "keyint=${GOP}:min-keyint=${GOP}:scenecut=0:bframes=0:log-level=error")
fi
