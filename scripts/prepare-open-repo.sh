...
OPEN_PATHS=(
  ...
  apps/ai
  apps/web
  # Public docs site (Docusaurus -> Cloudflare Workers); node_modules/build scrubbed below
  website
  # Reference sidecar plugin(s) — the SPI template
  examples
...
#   internal business/strategy docs                       (kept private)
...
# The module nav + routes are dynamic (driven by GET /api/v1/modules), so the open build simply never
# reports a bakery module; here we only drop its bundled page + the registry entries that import it.
# Bakery lines in modules.tsx are tagged `@module:bakery`; the icon fn is removed by its col-0 braces.
WEB="$TARGET/apps/web/src"
rm -f "$WEB/pages/Bakery.tsx"
sed -i '/@module:bakery/d; /^function BakeryIcon/,/^}/d' "$WEB/modules.tsx" 2>/dev/null || true
...
rm -rf "$TARGET/website/node_modules" "$TARGET/website/build" "$TARGET/website/.docusaurus"  # docs site: heavy/generated
...
[dependencies]
heldar-kernel = { path = "../heldar-kernel", version = "0.1.2" }
heldar-entry = { path = "../heldar-entry", version = "0.1.2" }
heldar-movement = { path = "../heldar-movement", version = "0.1.2" }
heldar-search = { path = "../heldar-search", version = "0.1.2" }
