...
REPORT="$DATA/web_smoke.txt"
LOG="$DATA/web_logs"; mkdir -p "$LOG"
: > "$REPORT"
...
sleep 2
HELDAR_DEFAULT_SEGMENT_SECONDS=10 HELDAR_DATA_DIR="$DATA" "$CORE" >"$LOG/core.log" 2>&1 & CORE_PID=$!
for _ in $(seq 1 30); do curl -fsS localhost:8000/healthz >/dev/null 2>&1 && break; sleep 1; done
# After the core: MediaMTX delegates publish auth to the kernel, so publishing first gets a 401.
