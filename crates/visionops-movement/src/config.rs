...
impl MovementConfig {
    pub fn from_env() -> Self {
        let red = std::env::var("VISIONOPS_MOVEMENT_RED_ZONE_KINDS")
            .ok()
            .filter(|s| !s.trim().is_empty())
            .unwrap_or_else(|| "restricted,red".to_string());
        let engine_interval_s = parse_or::<u64>("VISIONOPS_MOVEMENT_INTERVAL_S", 60).clamp(15, 600);
        ...
        let scan_window_s = parse_or::<i64>("VISIONOPS_MOVEMENT_SCAN_WINDOW_S", 900)
            .clamp(engine_interval_s as i64 * 2, 86_400);
        MovementConfig {
            engine_interval_s,
            scan_window_s,
            min_candidate_score: parse_or::<f64>("VISIONOPS_MOVEMENT_MIN_SCORE", 0.5)
                .clamp(0.0, 1.0),
            red_zone_kinds: red
                .split(',')
                ...
                .filter(|s| !s.is_empty())
                .collect(),
            retention_days: parse_or("VISIONOPS_MOVEMENT_RETENTION_DAYS", 365).clamp(1, 3650),
        }
    }
}
