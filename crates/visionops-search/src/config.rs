...
impl SearchConfig {
    pub fn from_env() -> Self {
        SearchConfig {
            llm_url: std::env::var("VISIONOPS_SEARCH_LLM_URL")
                .ok()
                .filter(|s| !s.trim().is_empty()),
            llm_api_key: std::env::var("VISIONOPS_SEARCH_LLM_API_KEY")
                .ok()
                .filter(|s| !s.trim().is_empty()),
            llm_model: std::env::var("VISIONOPS_SEARCH_LLM_MODEL")
                .ok()
                .filter(|s| !s.trim().is_empty())
                .unwrap_or_else(|| "gpt-4o-mini".to_string()),
            max_results: parse_or::<i64>("VISIONOPS_SEARCH_MAX_RESULTS", 200).clamp(1, 5000),
        }
    }
}
