import { useCallback as e, useEffect as t, useMemo as n, useRef as r, useState as i } from "react";
import { useNavigate as a } from "react-router-dom";
import { ApiError as o, Button as s, EmptyState as c, Field as l, Input as u, Login as d, Panel as f, SectionLabel as p, Select as m, Spinner as h, Stat as g, StatusPill as _, api as v, cx as y, formatClock as b, friendlyError as x, localInputToIso as S, setAuthToken as C, usePoll as w } from "@heldar/shell";
import { Fragment as T, jsx as E, jsxs as D } from "react/jsx-runtime";
//#region src/modules/search/page.tsx
var O = {
	matched: "recording",
	exception: "connecting",
	blocked: "error",
	unmatched: "offline"
}, k = {
	matched: "#10b981",
	exception: "#fbbf24",
	blocked: "#ef4444",
	unmatched: "#52525b"
}, A = {
	entry: "#38bdf8",
	zone: "#a78bfa",
	breach: "#ef4444"
}, j = {
	llm: "#f59e0b",
	rules: "#38bdf8",
	structured: "#a78bfa"
}, M = {
	inference: {
		color: "#fbbf24",
		blurb: "Interpretation — the only inference"
	},
	aggregate: {
		color: "#38bdf8",
		blurb: "Deterministic query over stored facts"
	},
	event: {
		color: "#10b981",
		blurb: "Event-level facts, each with provenance"
	},
	track: {
		color: "#10b981",
		blurb: "Track-level provenance"
	},
	observation: {
		color: "#10b981",
		blurb: "Raw observation"
	}
}, N = {
	inference: 0,
	aggregate: 1,
	event: 2,
	track: 3,
	observation: 4
}, P = [
	"white cars entering after 6pm last week",
	"unauthorized vehicles today",
	"red zone breaches yesterday"
];
function F({ label: e, color: t }) {
	return /* @__PURE__ */ E("span", {
		className: "inline-flex shrink-0 items-center rounded border px-1.5 py-0.5 font-mono text-[9px] font-semibold uppercase tracking-micro leading-none",
		style: {
			color: t,
			borderColor: `${t}55`,
			backgroundColor: `${t}1a`
		},
		children: e
	});
}
function I({ className: e }) {
	return /* @__PURE__ */ D("svg", {
		viewBox: "0 0 16 16",
		width: "14",
		height: "14",
		fill: "none",
		stroke: "currentColor",
		strokeWidth: "1.5",
		strokeLinecap: "round",
		strokeLinejoin: "round",
		"aria-hidden": "true",
		className: e,
		children: [
			/* @__PURE__ */ E("path", { d: "M8 1.5l6.5 11.5H1.5z" }),
			/* @__PURE__ */ E("path", { d: "M8 6.5v3.5" }),
			/* @__PURE__ */ E("path", { d: "M8 11.6v.4" })
		]
	});
}
function L({ children: e }) {
	return /* @__PURE__ */ D("div", {
		role: "alert",
		className: "flex items-start gap-2 rounded-md border border-danger/40 bg-danger/10 px-3 py-2 font-mono text-xs text-red-300",
		children: [/* @__PURE__ */ E(I, { className: "mt-0.5 shrink-0" }), /* @__PURE__ */ E("span", {
			className: "break-words",
			children: e
		})]
	});
}
function R(e, t) {
	if (!e) return null;
	let n = e[t];
	return typeof n == "string" ? n.trim() ? n : null : typeof n == "number" || typeof n == "boolean" ? String(n) : null;
}
function z({ path: e, alt: t }) {
	let [n, r] = i(!1);
	return n ? null : /* @__PURE__ */ E("img", {
		src: e,
		alt: t,
		loading: "lazy",
		onError: () => r(!0),
		className: "h-16 w-24 shrink-0 rounded-md border border-line bg-black object-cover"
	});
}
function B({ children: e }) {
	return /* @__PURE__ */ D("div", {
		className: "flex items-start gap-3 rounded-panel border border-line bg-panel px-4 py-3",
		children: [/* @__PURE__ */ D("svg", {
			viewBox: "0 0 20 20",
			className: "mt-0.5 h-4 w-4 shrink-0 text-accent",
			fill: "none",
			stroke: "currentColor",
			strokeWidth: "1.6",
			strokeLinecap: "round",
			strokeLinejoin: "round",
			"aria-hidden": "true",
			children: [
				/* @__PURE__ */ E("circle", {
					cx: "10",
					cy: "10",
					r: "7.5"
				}),
				/* @__PURE__ */ E("path", { d: "M10 9v4" }),
				/* @__PURE__ */ E("path", { d: "M10 6.6v.4" })
			]
		}), /* @__PURE__ */ E("p", {
			className: "font-mono text-[11px] leading-relaxed text-fg-secondary",
			children: e
		})]
	});
}
function V(e) {
	return `${String(e).padStart(2, "0")}:00`;
}
function H({ label: e, value: t }) {
	return /* @__PURE__ */ D("span", {
		className: "inline-flex items-center gap-1.5 rounded-md border border-line bg-canvas px-2 py-1 leading-none",
		children: [/* @__PURE__ */ E("span", {
			className: "font-mono text-[9px] uppercase tracking-micro text-fg-muted",
			children: e
		}), /* @__PURE__ */ E("span", {
			className: "font-mono text-[11px] font-semibold text-fg",
			children: t
		})]
	});
}
function U(e, t) {
	let n = [];
	return e.from && n.push({
		label: "From",
		value: b(e.from)
	}), e.to && n.push({
		label: "To",
		value: b(e.to)
	}), e.hour_min != null && n.push({
		label: "After",
		value: `${V(e.hour_min)} UTC`
	}), e.hour_max != null && n.push({
		label: "Before",
		value: `${V(e.hour_max)} UTC`
	}), e.cameras && e.cameras.length > 0 && n.push({
		label: "Cameras",
		value: e.cameras.map((e) => t(e)).join(", ")
	}), e.sources && e.sources.length > 0 && n.push({
		label: "Sources",
		value: e.sources.join(" · ")
	}), e.plate && n.push({
		label: "Plate",
		value: e.plate
	}), e.color && n.push({
		label: "Color",
		value: e.color
	}), e.vehicle_type && n.push({
		label: "Vehicle",
		value: e.vehicle_type
	}), e.subject_type && n.push({
		label: "Subject",
		value: e.subject_type
	}), e.auth_status && e.auth_status.length > 0 && n.push({
		label: "Auth",
		value: e.auth_status.join(" · ")
	}), e.event_type && n.push({
		label: "Event",
		value: e.event_type
	}), e.zone_kind && n.push({
		label: "Zone kind",
		value: e.zone_kind
	}), e.text && n.push({
		label: "Text",
		value: e.text
	}), e.limit != null && n.push({
		label: "Limit",
		value: String(e.limit)
	}), n;
}
function W({ plan: e, planner: t, nameFor: r, dryRun: i }) {
	let a = n(() => U(e, r), [e, r]), o = j[t] ?? "#71717a";
	return /* @__PURE__ */ D(f, {
		title: "Interpreted as",
		subtitle: "The structured plan your question was turned into — the only inference in the answer",
		actions: /* @__PURE__ */ D("div", {
			className: "flex items-center gap-2",
			children: [i && /* @__PURE__ */ E(F, {
				label: "Dry run",
				color: "#fbbf24"
			}), /* @__PURE__ */ E(F, {
				label: `Planner · ${t}`,
				color: o
			})]
		}),
		children: [a.length === 0 ? /* @__PURE__ */ D("p", {
			className: "font-mono text-[11px] leading-relaxed text-fg-secondary",
			children: [
				"No filters were extracted — this defaults to",
				" ",
				/* @__PURE__ */ E("span", {
					className: "text-fg",
					children: "all sources"
				}),
				" over the last ~7 days. Add detail (color, time, camera, authorization) to narrow it."
			]
		}) : /* @__PURE__ */ E("div", {
			className: "flex flex-wrap gap-2",
			children: a.map((e) => /* @__PURE__ */ E(H, {
				label: e.label,
				value: e.value
			}, e.label))
		}), /* @__PURE__ */ D("p", {
			className: "mt-3 flex items-start gap-1.5 border-t border-line pt-3 font-mono text-[10px] leading-relaxed text-fg-muted",
			children: [/* @__PURE__ */ E(I, { className: "mt-0.5 shrink-0 text-fg-muted/80" }), /* @__PURE__ */ D("span", { children: [
				"Verify this reflects your intent — the planner only decides",
				" ",
				/* @__PURE__ */ E("span", {
					className: "text-fg-secondary",
					children: "how to query"
				}),
				". The results are exactly what this plan selected, nothing more."
			] })]
		})]
	});
}
function G(e, t) {
	let n = e[t];
	return typeof n == "string" && n.trim() ? n : null;
}
function ee({ proof: e }) {
	let t = n(() => {
		let t = [...e.claim_levels ?? []];
		return t.sort((e, t) => (N[String(e.level ?? "")] ?? 99) - (N[String(t.level ?? "")] ?? 99)), t;
	}, [e.claim_levels]);
	return /* @__PURE__ */ D(f, {
		title: "Proof",
		subtitle: "Why this answer can be trusted — facts at the bottom, interpretation at the top",
		children: [
			/* @__PURE__ */ D("p", {
				className: "mb-4 rounded-md border border-accent/30 bg-accent/[0.06] px-3 py-2 text-xs leading-relaxed text-fg-secondary",
				children: [
					/* @__PURE__ */ E("span", {
						className: "font-semibold text-fg",
						children: "The answers are facts; the interpretation is the only inference."
					}),
					" ",
					"Each rung below states a claim, its confidence, and the caveat that bounds it."
				]
			}),
			/* @__PURE__ */ D("ol", {
				className: "relative space-y-3 pl-5",
				children: [/* @__PURE__ */ E("span", {
					className: "absolute left-[5px] top-2 bottom-2 w-px bg-line",
					"aria-hidden": "true"
				}), t.map((e, t) => {
					let n = G(e, "level") ?? "—", r = M[n] ?? {
						color: "#71717a",
						blurb: ""
					}, i = G(e, "statement"), a = G(e, "confidence"), o = G(e, "caveat"), s = G(e, "basis"), c = G(e, "provenance");
					return /* @__PURE__ */ D("li", {
						className: "relative",
						children: [/* @__PURE__ */ E("span", {
							className: "absolute -left-5 top-1.5 h-2.5 w-2.5 rounded-full border-2 border-canvas",
							style: { backgroundColor: r.color },
							"aria-hidden": "true"
						}), /* @__PURE__ */ D("div", {
							className: "rounded-md border border-line bg-panel2/40 p-3",
							style: {
								borderLeftColor: r.color,
								borderLeftWidth: 3
							},
							children: [
								/* @__PURE__ */ D("div", {
									className: "flex flex-wrap items-center gap-2",
									children: [
										/* @__PURE__ */ E(F, {
											label: n,
											color: r.color
										}),
										r.blurb && /* @__PURE__ */ E("span", {
											className: "font-mono text-[10px] text-fg-muted",
											children: r.blurb
										}),
										a && /* @__PURE__ */ D("span", {
											className: "ml-auto whitespace-nowrap font-mono text-[10px] text-fg-secondary",
											children: ["confidence:\xA0", /* @__PURE__ */ E("span", {
												className: "text-fg",
												children: a
											})]
										})
									]
								}),
								i && /* @__PURE__ */ E("p", {
									className: "mt-2 text-xs leading-relaxed text-fg-secondary",
									children: i
								}),
								s && /* @__PURE__ */ D("p", {
									className: "mt-1.5 font-mono text-[10px] leading-relaxed text-fg-muted",
									children: ["basis: ", s]
								}),
								c && /* @__PURE__ */ D("p", {
									className: "mt-1.5 font-mono text-[10px] leading-relaxed text-fg-muted",
									children: ["provenance: ", c]
								}),
								o && /* @__PURE__ */ D("p", {
									className: "mt-2 flex items-start gap-1.5 rounded border border-connecting/30 bg-connecting/[0.06] px-2 py-1.5 font-mono text-[10px] leading-relaxed text-connecting",
									children: [/* @__PURE__ */ E(I, { className: "mt-0.5 shrink-0" }), /* @__PURE__ */ E("span", { children: o })]
								})
							]
						})]
					}, `${n}-${t}`);
				})]
			}),
			e.note && /* @__PURE__ */ E("p", {
				className: "mt-4 border-t border-line pt-3 font-mono text-[10px] leading-relaxed text-fg-muted",
				children: e.note
			})
		]
	});
}
function K({ hit: e, nameFor: t }) {
	let n = A[e.source] ?? "#71717a", r = (e.auth_status ? k[e.auth_status] : void 0) ?? n, i = R(e.subject, "color"), a = R(e.subject, "vehicle_type"), o = R(e.subject, "label"), s = R(e.subject, "subject_type") ?? R(e.subject, "type"), c = R(e.subject, "severity");
	return /* @__PURE__ */ D("div", {
		className: "flex gap-3 rounded-md border border-line bg-panel2/40 p-3 transition-colors duration-150 hover:border-[#34373e]",
		style: {
			borderLeftColor: r,
			borderLeftWidth: 3
		},
		children: [e.evidence_path && /* @__PURE__ */ E(z, {
			path: e.evidence_path,
			alt: `${e.source} ${e.plate ?? e.id}`
		}), /* @__PURE__ */ D("div", {
			className: "min-w-0 flex-1",
			children: [
				/* @__PURE__ */ D("div", {
					className: "flex flex-wrap items-center gap-2",
					children: [
						/* @__PURE__ */ E(F, {
							label: e.source,
							color: n
						}),
						/* @__PURE__ */ E("span", {
							className: "font-mono text-[10px] uppercase tracking-micro text-fg-muted",
							children: e.kind
						}),
						e.claim_level && /* @__PURE__ */ E(F, {
							label: e.claim_level,
							color: "#52525b"
						}),
						/* @__PURE__ */ E("span", {
							className: "ml-auto whitespace-nowrap font-mono text-[10px] text-fg-muted",
							children: b(e.timestamp)
						})
					]
				}),
				/* @__PURE__ */ D("div", {
					className: "mt-2 flex flex-wrap items-center gap-2",
					children: [e.plate ? /* @__PURE__ */ E("span", {
						className: "font-mono text-base font-semibold tracking-wide text-fg",
						children: e.plate
					}) : /* @__PURE__ */ E("span", {
						className: "font-mono text-sm text-fg-secondary",
						children: o ?? s ?? "—"
					}), e.auth_status && /* @__PURE__ */ E(_, {
						state: O[e.auth_status] ?? "unknown",
						label: e.auth_status
					})]
				}),
				/* @__PURE__ */ D("div", {
					className: "mt-1.5 flex flex-wrap gap-x-3 gap-y-0.5 font-mono text-[10px] text-fg-secondary",
					children: [
						/* @__PURE__ */ D("span", {
							className: "text-fg-muted",
							children: ["camera:\xA0", /* @__PURE__ */ E("span", {
								className: "text-fg-secondary",
								children: t(e.camera_id)
							})]
						}),
						e.zone && /* @__PURE__ */ D("span", {
							className: "text-fg-muted",
							children: [
								"zone:\xA0",
								/* @__PURE__ */ E("span", {
									className: "text-fg-secondary",
									children: e.zone
								}),
								e.zone_kind ? /* @__PURE__ */ D("span", {
									className: "text-fg-muted",
									children: [
										" (",
										e.zone_kind,
										")"
									]
								}) : null
							]
						}),
						s && e.plate && /* @__PURE__ */ D("span", {
							className: "text-fg-muted",
							children: ["subject:\xA0", /* @__PURE__ */ E("span", {
								className: "text-fg-secondary",
								children: s
							})]
						}),
						a && /* @__PURE__ */ D("span", {
							className: "text-fg-muted",
							children: ["type:\xA0", /* @__PURE__ */ E("span", {
								className: "text-fg-secondary",
								children: a
							})]
						}),
						i && /* @__PURE__ */ D("span", {
							className: "text-fg-muted",
							children: ["color:\xA0", /* @__PURE__ */ E("span", {
								className: "text-fg-secondary",
								children: i
							})]
						}),
						c && /* @__PURE__ */ D("span", {
							className: "text-fg-muted",
							children: ["severity:\xA0", /* @__PURE__ */ E("span", {
								className: "text-fg-secondary",
								children: c
							})]
						})
					]
				})
			]
		})]
	});
}
function q({ result: e, nameFor: t }) {
	let r = n(() => {
		let t = 0, n = 0, r = 0;
		for (let i of e.hits) i.source === "entry" ? t += 1 : i.source === "zone" ? n += 1 : i.source === "breach" && (r += 1);
		return {
			entry: t,
			zone: n,
			breach: r
		};
	}, [e.hits]);
	return /* @__PURE__ */ D(T, { children: [/* @__PURE__ */ D("div", {
		className: "grid grid-cols-2 gap-px overflow-hidden rounded-panel border border-line bg-line sm:grid-cols-4",
		children: [
			/* @__PURE__ */ E("div", {
				className: "bg-panel px-4 py-3",
				children: /* @__PURE__ */ E(g, {
					label: "Matches",
					value: e.count
				})
			}),
			/* @__PURE__ */ E("div", {
				className: "bg-panel px-4 py-3",
				children: /* @__PURE__ */ E(g, {
					label: "Entry",
					value: r.entry
				})
			}),
			/* @__PURE__ */ E("div", {
				className: "bg-panel px-4 py-3",
				children: /* @__PURE__ */ E(g, {
					label: "Zone",
					value: r.zone
				})
			}),
			/* @__PURE__ */ E("div", {
				className: "bg-panel px-4 py-3",
				children: /* @__PURE__ */ E(g, {
					label: "Breach",
					value: r.breach,
					tone: r.breach > 0 ? "bad" : "default"
				})
			})
		]
	}), /* @__PURE__ */ E(f, {
		title: "Results",
		subtitle: "Stored events matching the executed plan — newest first",
		actions: /* @__PURE__ */ E("span", {
			className: "font-mono text-[11px] tabular-nums text-fg-muted",
			children: e.count
		}),
		children: e.hits.length === 0 ? /* @__PURE__ */ E(c, {
			title: "No matching events",
			hint: "The plan ran cleanly but no stored events matched. Loosen the filters above, widen the time window, or check the interpreted plan."
		}) : /* @__PURE__ */ E("div", {
			className: "space-y-2.5",
			children: e.hits.map((e) => /* @__PURE__ */ E(K, {
				hit: e,
				nameFor: t
			}, `${e.source}-${e.id}`))
		})
	})] });
}
function J({ busy: e, onRun: t }) {
	let [n, r] = i(""), [a, o] = i(""), [c, d] = i(""), [f, p] = i(""), [g, _] = i("");
	function v(e) {
		e.preventDefault();
		let r = {};
		n && (r.sources = [n]), a && (r.auth_status = [a]), c.trim() && (r.color = c.trim());
		let i = S(f);
		i && (r.from = i);
		let o = S(g);
		o && (r.to = o), t(r);
	}
	return /* @__PURE__ */ D("form", {
		onSubmit: v,
		className: "space-y-4",
		children: [/* @__PURE__ */ D("div", {
			className: "grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3",
			children: [
				/* @__PURE__ */ E(l, {
					label: "Source",
					htmlFor: "sf-source",
					children: /* @__PURE__ */ D(m, {
						id: "sf-source",
						value: n,
						onChange: (e) => r(e.target.value),
						children: [
							/* @__PURE__ */ E("option", {
								value: "",
								children: "Any source"
							}),
							/* @__PURE__ */ E("option", {
								value: "entry",
								children: "Entry"
							}),
							/* @__PURE__ */ E("option", {
								value: "zone",
								children: "Zone"
							}),
							/* @__PURE__ */ E("option", {
								value: "breach",
								children: "Breach"
							})
						]
					})
				}),
				/* @__PURE__ */ E(l, {
					label: "Authorization",
					htmlFor: "sf-auth",
					children: /* @__PURE__ */ D(m, {
						id: "sf-auth",
						value: a,
						onChange: (e) => o(e.target.value),
						children: [
							/* @__PURE__ */ E("option", {
								value: "",
								children: "Any status"
							}),
							/* @__PURE__ */ E("option", {
								value: "matched",
								children: "Matched"
							}),
							/* @__PURE__ */ E("option", {
								value: "exception",
								children: "Exception"
							}),
							/* @__PURE__ */ E("option", {
								value: "unmatched",
								children: "Unmatched"
							}),
							/* @__PURE__ */ E("option", {
								value: "blocked",
								children: "Blocked"
							})
						]
					})
				}),
				/* @__PURE__ */ E(l, {
					label: "Color",
					htmlFor: "sf-color",
					children: /* @__PURE__ */ E(u, {
						id: "sf-color",
						value: c,
						onChange: (e) => d(e.target.value),
						placeholder: "white",
						autoComplete: "off"
					})
				}),
				/* @__PURE__ */ E(l, {
					label: "From",
					htmlFor: "sf-from",
					children: /* @__PURE__ */ E(u, {
						id: "sf-from",
						type: "datetime-local",
						step: 1,
						value: f,
						onChange: (e) => p(e.target.value)
					})
				}),
				/* @__PURE__ */ E(l, {
					label: "To",
					htmlFor: "sf-to",
					children: /* @__PURE__ */ E(u, {
						id: "sf-to",
						type: "datetime-local",
						step: 1,
						value: g,
						onChange: (e) => _(e.target.value)
					})
				})
			]
		}), /* @__PURE__ */ E("div", {
			className: "flex justify-end",
			children: /* @__PURE__ */ E(s, {
				type: "submit",
				variant: "primary",
				disabled: e,
				children: e ? /* @__PURE__ */ D(T, { children: [/* @__PURE__ */ E(h, { size: 14 }), "Running…"] }) : "Run structured query"
			})
		})]
	});
}
var te = [
	12,
	24,
	48
];
function Y({ path: e, alt: t }) {
	let [n, r] = i(!1);
	return n ? null : /* @__PURE__ */ E("a", {
		href: e,
		target: "_blank",
		rel: "noreferrer",
		className: "group shrink-0 self-start",
		title: "Open crop",
		children: /* @__PURE__ */ E("img", {
			src: e,
			alt: t,
			loading: "lazy",
			onError: () => r(!0),
			className: "h-28 w-40 rounded-md border border-line bg-black object-cover transition-colors duration-150 group-hover:border-accent"
		})
	});
}
function ne({ hit: e, rank: t, nameFor: n, onPlayback: r }) {
	return /* @__PURE__ */ D("div", {
		className: "flex gap-3 rounded-md border border-line bg-panel2/40 p-3 transition-colors duration-150 hover:border-[#34373e]",
		children: [e.evidence_path && /* @__PURE__ */ E(Y, {
			path: e.evidence_path,
			alt: e.label ?? "match"
		}), /* @__PURE__ */ D("div", {
			className: "flex min-w-0 flex-1 flex-col",
			children: [
				/* @__PURE__ */ D("div", {
					className: "flex flex-wrap items-center gap-2",
					children: [
						/* @__PURE__ */ D("span", {
							className: "font-mono text-[10px] tabular-nums text-fg-muted",
							children: ["#", t]
						}),
						/* @__PURE__ */ E(F, {
							label: `${(e.score * 100).toFixed(0)}%`,
							color: "#a78bfa"
						}),
						/* @__PURE__ */ E("span", {
							className: "ml-auto whitespace-nowrap font-mono text-[10px] text-fg-muted",
							children: b(e.timestamp)
						})
					]
				}),
				/* @__PURE__ */ E("div", {
					className: "mt-2 truncate font-mono text-sm text-fg",
					children: e.label ?? "—"
				}),
				/* @__PURE__ */ D("div", {
					className: "mt-1 flex flex-wrap gap-x-3 gap-y-0.5 font-mono text-[10px] text-fg-muted",
					children: [
						/* @__PURE__ */ D("span", { children: ["camera:\xA0", /* @__PURE__ */ E("span", {
							className: "text-fg-secondary",
							children: n(e.camera_id)
						})] }),
						e.track_id && /* @__PURE__ */ D("span", { children: ["track:\xA0", /* @__PURE__ */ D("span", {
							className: "text-fg-secondary",
							children: ["#", e.track_id]
						})] }),
						e.detection?.confidence != null && /* @__PURE__ */ D("span", { children: ["det\xA0conf:\xA0", /* @__PURE__ */ E("span", {
							className: "text-fg-secondary",
							children: e.detection.confidence.toFixed(2)
						})] })
					]
				}),
				/* @__PURE__ */ E("div", {
					className: "mt-auto pt-2",
					children: /* @__PURE__ */ E(s, {
						size: "sm",
						onClick: () => r(e),
						children: "Playback"
					})
				})
			]
		})]
	});
}
function X({ nameFor: e, cameras: n }) {
	let d = a(), p = r(null), [g, _] = i(""), [y, b] = i(null), [C, w] = i(null), [O, k] = i(null), [A, j] = i(""), [M, N] = i(""), [P, F] = i(""), [R, z] = i(""), [V, H] = i(""), [U, W] = i([]), [G, ee] = i(24);
	t(() => {
		if (H(""), !P) {
			W([]);
			return;
		}
		let e = !1;
		return v.listZones(P).then((t) => {
			e || W(t);
		}).catch(() => {
			e || W([]);
		}), () => {
			e = !0;
		};
	}, [P]);
	let [K, q] = i(null), [J, Y] = i(null), [X, Z] = i(null), [re, Q] = i(!1), $ = g.trim().length > 0;
	function ie(e) {
		if (!e || $) return;
		if (!e.type.startsWith("image/")) {
			Y(`"${e.name}" is not an image — drop a JPEG, PNG, WebP, or GIF.`);
			return;
		}
		if (e.size === 0) {
			Y(`"${e.name}" is empty.`);
			return;
		}
		if (e.size > 7340032) {
			Y(`"${e.name}" is ${(e.size / 1048576).toFixed(1)} MB — the limit is 7 MB. Resize or crop it first.`);
			return;
		}
		let t = new FileReader();
		t.onload = () => {
			let n = typeof t.result == "string" ? t.result : "", r = n.indexOf(",");
			if (r < 0) {
				Y("Could not read that file as an image.");
				return;
			}
			b(n.slice(r + 1)), k(n), w(e.name), Y(null);
		}, t.onerror = () => Y("Could not read that file as an image."), t.readAsDataURL(e);
	}
	function ae() {
		b(null), w(null), k(null), p.current && (p.current.value = "");
	}
	async function oe(e) {
		e.preventDefault();
		let t = g.trim();
		if (!t && !y) return;
		q("semantic"), Y(null);
		let n = t ? { text: t } : { image_b64: y }, r = S(A);
		r && (n.from = r);
		let i = S(M);
		i && (n.to = i), P && (n.cameras = [P]);
		let a = R.trim();
		a && (n.label = a), V && (n.zone = V), n.k = G;
		try {
			let e = await v.searchSemantic(n);
			Z(e), Q(!0);
		} catch (e) {
			e instanceof o && e.status === 503 ? Y("Embedding worker offline — semantic search needs a running AI worker with the CLIP extra installed.") : Y(x(e)), Z(null), Q(!0);
		} finally {
			q(null);
		}
	}
	function se(e) {
		let t = new Date(e.timestamp).getTime(), n = (/* @__PURE__ */ new Date(t - 6e4)).toISOString(), r = new Date(t + 6e4).toISOString();
		d(`/playback?camera=${encodeURIComponent(e.camera_id)}&from=${encodeURIComponent(n)}&to=${encodeURIComponent(r)}`);
	}
	return /* @__PURE__ */ D("div", {
		className: "stagger space-y-4",
		children: [
			/* @__PURE__ */ D(B, { children: [
				/* @__PURE__ */ E("span", {
					className: "text-fg",
					children: "Similarity-ranked, not verified."
				}),
				" These results are CLIP embedding matches — detection crops ranked by how visually close they are to your text or example image. A score is a relative rank,",
				" ",
				/* @__PURE__ */ E("span", {
					className: "text-fg",
					children: "not a probability and not a stored fact"
				}),
				" — confirm anything that matters in Playback before acting on it."
			] }),
			/* @__PURE__ */ D(f, {
				title: "Find",
				subtitle: "Describe what you're looking for, or match an example image",
				children: [/* @__PURE__ */ D("form", {
					onSubmit: oe,
					className: "space-y-4",
					children: [
						/* @__PURE__ */ D("div", {
							className: "grid grid-cols-1 gap-3 lg:grid-cols-2",
							children: [/* @__PURE__ */ E(l, {
								label: "Describe it",
								htmlFor: "sem-text",
								children: /* @__PURE__ */ E(u, {
									id: "sem-text",
									value: g,
									onChange: (e) => _(e.target.value),
									placeholder: "red pickup truck",
									autoComplete: "off",
									disabled: y != null
								})
							}), /* @__PURE__ */ D(l, {
								label: "Or match an image",
								htmlFor: "sem-image",
								children: [/* @__PURE__ */ E("input", {
									ref: p,
									id: "sem-image",
									type: "file",
									accept: "image/*",
									className: "hidden",
									disabled: K !== null || $,
									onChange: (e) => {
										ie(e.target.files?.[0]), e.target.value = "";
									}
								}), y ? /* @__PURE__ */ D("div", {
									className: "flex items-center gap-2 rounded-md border border-line bg-panel2 px-2 py-1.5",
									children: [
										O && /* @__PURE__ */ E("img", {
											src: O,
											alt: C ?? "query image",
											className: "h-9 w-14 shrink-0 rounded border border-line bg-black object-cover"
										}),
										/* @__PURE__ */ E("span", {
											className: "min-w-0 flex-1 truncate font-mono text-[11px] text-fg-secondary",
											children: C
										}),
										/* @__PURE__ */ E(s, {
											size: "sm",
											onClick: ae,
											disabled: K !== null,
											children: "Clear"
										})
									]
								}) : /* @__PURE__ */ D("button", {
									type: "button",
									onClick: () => p.current?.click(),
									disabled: K !== null || $,
									onDragOver: (e) => e.preventDefault(),
									onDrop: (e) => {
										e.preventDefault(), $ || ie(e.dataTransfer.files?.[0]);
									},
									className: "flex w-full items-center justify-center gap-2 rounded-md border border-dashed border-line bg-panel2 px-3 py-2 font-mono text-[11px] text-fg-muted transition-colors duration-150 hover:border-accent/50 hover:text-fg-secondary disabled:cursor-not-allowed disabled:opacity-50",
									children: [/* @__PURE__ */ D("svg", {
										viewBox: "0 0 16 16",
										width: "12",
										height: "12",
										fill: "none",
										stroke: "currentColor",
										strokeWidth: "1.5",
										strokeLinecap: "round",
										strokeLinejoin: "round",
										"aria-hidden": "true",
										children: [
											/* @__PURE__ */ E("path", { d: "M8 10.5V3" }),
											/* @__PURE__ */ E("path", { d: "M5 5.5l3-3 3 3" }),
											/* @__PURE__ */ E("path", { d: "M2.5 10.5v2a1 1 0 0 0 1 1h9a1 1 0 0 0 1-1v-2" })
										]
									}), $ ? "Text query set — clear it to use an image" : "Drop an image or click to browse"]
								})]
							})]
						}),
						/* @__PURE__ */ D("div", {
							className: "grid grid-cols-2 gap-3 sm:grid-cols-4",
							children: [
								/* @__PURE__ */ E(l, {
									label: "From",
									htmlFor: "sem-from",
									children: /* @__PURE__ */ E(u, {
										id: "sem-from",
										type: "datetime-local",
										step: 1,
										value: A,
										onChange: (e) => j(e.target.value)
									})
								}),
								/* @__PURE__ */ E(l, {
									label: "To",
									htmlFor: "sem-to",
									children: /* @__PURE__ */ E(u, {
										id: "sem-to",
										type: "datetime-local",
										step: 1,
										value: M,
										onChange: (e) => N(e.target.value)
									})
								}),
								/* @__PURE__ */ E(l, {
									label: "Camera",
									htmlFor: "sem-camera",
									children: /* @__PURE__ */ D(m, {
										id: "sem-camera",
										value: P,
										onChange: (e) => F(e.target.value),
										children: [/* @__PURE__ */ E("option", {
											value: "",
											children: "All cameras"
										}), n.map((e) => /* @__PURE__ */ E("option", {
											value: e.id,
											children: e.name
										}, e.id))]
									})
								}),
								/* @__PURE__ */ E(l, {
									label: "Zone (optional)",
									htmlFor: "sem-zone",
									children: /* @__PURE__ */ D(m, {
										id: "sem-zone",
										value: V,
										onChange: (e) => H(e.target.value),
										disabled: !P || U.length === 0,
										title: P ? void 0 : "Pick a camera first — zones are per-camera",
										children: [/* @__PURE__ */ E("option", {
											value: "",
											children: P ? "Whole frame" : "Pick a camera first"
										}), U.map((e) => /* @__PURE__ */ E("option", {
											value: e.id,
											children: e.name
										}, e.id))]
									})
								}),
								/* @__PURE__ */ E(l, {
									label: "Label (optional)",
									htmlFor: "sem-label",
									children: /* @__PURE__ */ E(u, {
										id: "sem-label",
										value: R,
										onChange: (e) => z(e.target.value),
										placeholder: "car",
										autoComplete: "off"
									})
								}),
								/* @__PURE__ */ E(l, {
									label: "Results",
									htmlFor: "sem-k",
									children: /* @__PURE__ */ E(m, {
										id: "sem-k",
										value: G,
										onChange: (e) => ee(Number(e.target.value)),
										children: te.map((e) => /* @__PURE__ */ D("option", {
											value: e,
											children: ["Top ", e]
										}, e))
									})
								})
							]
						}),
						/* @__PURE__ */ E("div", {
							className: "flex justify-end",
							children: /* @__PURE__ */ E(s, {
								type: "submit",
								variant: "primary",
								disabled: K !== null || !$ && !y,
								children: K === "semantic" ? /* @__PURE__ */ D(T, { children: [/* @__PURE__ */ E(h, { size: 14 }), "Searching…"] }) : "Search by similarity"
							})
						})
					]
				}), J && /* @__PURE__ */ E("div", {
					className: "mt-3",
					children: /* @__PURE__ */ E(L, { children: J })
				})]
			}),
			X && /* @__PURE__ */ D(f, {
				title: "Ranked matches",
				subtitle: `Crops ranked by similarity to "${X.query}"${X.model ? ` · ${X.model}` : ""}`,
				actions: /* @__PURE__ */ E("span", {
					className: "font-mono text-[11px] tabular-nums text-fg-muted",
					children: X.count
				}),
				children: [X.truncated && /* @__PURE__ */ D("p", {
					className: "mb-3 flex items-start gap-1.5 rounded border border-connecting/30 bg-connecting/[0.06] px-2 py-1.5 font-mono text-[10px] leading-relaxed text-connecting",
					children: [/* @__PURE__ */ E(I, { className: "mt-0.5 shrink-0" }), /* @__PURE__ */ E("span", { children: "The candidate scan hit its cap before covering the whole window — narrow the time range or camera filter for a complete ranking." })]
				}), X.hits.length === 0 ? /* @__PURE__ */ E(c, {
					title: "No similar crops found",
					hint: "Nothing embedded in this window resembles the query. Widen the time range, check the camera filter, or confirm an embedding AI task is running for these cameras."
				}) : /* @__PURE__ */ E("div", {
					className: "grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3",
					children: X.hits.map((t, n) => /* @__PURE__ */ E(ne, {
						hit: t,
						rank: n + 1,
						nameFor: e,
						onPlayback: se
					}, t.id))
				})]
			}),
			!re && /* @__PURE__ */ E(c, {
				title: "Search by what it looks like",
				hint: "Type a description ('red pickup truck') or drop an example image. Crops from the embedding index are ranked by visual similarity — a recall tool for finding footage, not a source of verified facts."
			})
		]
	});
}
function Z({ nameFor: t }) {
	let [n, r] = i(""), [a, d] = i(null), [p, m] = i(null), [g, _] = i(null), [b, x] = i(null), [S, C] = i(!1), [w, O] = i(!1), k = e(async (e) => {
		let t = e.trim();
		if (t) {
			r(t), _("nl"), x(null), m(null);
			try {
				let e = await v.searchNl(t);
				d(e), C(!0);
			} catch (e) {
				x(e instanceof o ? e.message : String(e)), d(null), C(!0);
			} finally {
				_(null);
			}
		}
	}, []);
	async function A() {
		let e = n.trim();
		if (e) {
			_("plan"), x(null), d(null);
			try {
				let t = await v.searchPlan(e);
				m(t), C(!0);
			} catch (e) {
				x(e instanceof o ? e.message : String(e)), m(null), C(!0);
			} finally {
				_(null);
			}
		}
	}
	let j = e(async (e) => {
		_("structured"), x(null), m(null);
		try {
			let t = await v.searchEvents(e);
			d(t), C(!0);
		} catch (e) {
			x(e instanceof o ? e.message : String(e)), d(null), C(!0);
		} finally {
			_(null);
		}
	}, []);
	function M(e) {
		e.preventDefault(), k(n);
	}
	let N = p?.plan ?? a?.plan ?? null, F = p?.planner ?? a?.planner ?? "rules";
	return /* @__PURE__ */ D("div", {
		className: "stagger space-y-4",
		children: [
			/* @__PURE__ */ D(B, { children: [
				/* @__PURE__ */ E("span", {
					className: "text-fg",
					children: "Ask in plain language; the answer is the data."
				}),
				" A planner (transparent rules, or an optional LLM) translates your question into a structured query — that interpretation is the only inference. The plan then runs deterministically over the kernel's stored events, so",
				" ",
				/* @__PURE__ */ E("span", {
					className: "text-fg",
					children: "the answers are facts, the interpretation is the only inference"
				}),
				". Every search is logged; plate-targeted queries are audited."
			] }),
			/* @__PURE__ */ D(f, {
				title: "Ask",
				subtitle: "Natural-language search over entry, zone & breach events",
				children: [
					/* @__PURE__ */ D("form", {
						onSubmit: M,
						className: "flex flex-col gap-3 sm:flex-row sm:items-end",
						children: [/* @__PURE__ */ E("div", {
							className: "min-w-0 flex-1",
							children: /* @__PURE__ */ E(l, {
								label: "Query",
								htmlFor: "nl-query",
								children: /* @__PURE__ */ E(u, {
									id: "nl-query",
									value: n,
									onChange: (e) => r(e.target.value),
									placeholder: "white cars entering after 6pm last week",
									autoComplete: "off"
								})
							})
						}), /* @__PURE__ */ D("div", {
							className: "flex shrink-0 items-center gap-2",
							children: [/* @__PURE__ */ E(s, {
								type: "submit",
								variant: "primary",
								disabled: g !== null || !n.trim(),
								children: g === "nl" ? /* @__PURE__ */ D(T, { children: [/* @__PURE__ */ E(h, { size: 14 }), "Searching…"] }) : "Search"
							}), /* @__PURE__ */ E(s, {
								type: "button",
								onClick: () => void A(),
								disabled: g !== null || !n.trim(),
								children: g === "plan" ? /* @__PURE__ */ D(T, { children: [/* @__PURE__ */ E(h, { size: 14 }), "Planning…"] }) : "Plan only (dry-run)"
							})]
						})]
					}),
					/* @__PURE__ */ D("div", {
						className: "mt-3 flex flex-wrap items-center gap-2",
						children: [/* @__PURE__ */ E("span", {
							className: "font-mono text-[9px] uppercase tracking-micro text-fg-muted",
							children: "Try"
						}), P.map((e) => /* @__PURE__ */ E("button", {
							type: "button",
							disabled: g !== null,
							onClick: () => void k(e),
							className: "rounded-full border border-line bg-canvas px-2.5 py-1 font-mono text-[10px] text-fg-secondary transition-colors duration-150 hover:border-accent/50 hover:text-fg disabled:cursor-not-allowed disabled:opacity-50",
							children: e
						}, e))]
					}),
					/* @__PURE__ */ D("p", {
						className: "mt-3 flex items-center gap-1.5 font-mono text-[10px] uppercase tracking-micro text-fg-muted",
						children: [/* @__PURE__ */ D("svg", {
							viewBox: "0 0 16 16",
							width: "12",
							height: "12",
							fill: "none",
							stroke: "currentColor",
							strokeWidth: "1.5",
							"aria-hidden": "true",
							children: [/* @__PURE__ */ E("rect", {
								x: "3",
								y: "7",
								width: "10",
								height: "7",
								rx: "1.5"
							}), /* @__PURE__ */ E("path", {
								d: "M5.5 7V5a2.5 2.5 0 0 1 5 0v2",
								strokeLinecap: "round"
							})]
						}), "Searches are logged · plate-targeted queries are audited."]
					}),
					b && /* @__PURE__ */ E("div", {
						className: "mt-3",
						children: /* @__PURE__ */ E(L, { children: b })
					}),
					/* @__PURE__ */ D("div", {
						className: "mt-4 border-t border-line pt-3",
						children: [/* @__PURE__ */ D("button", {
							type: "button",
							onClick: () => O((e) => !e),
							className: "flex items-center gap-1.5 font-mono text-[10px] font-semibold uppercase tracking-micro text-fg-secondary transition-colors duration-150 hover:text-fg",
							children: [/* @__PURE__ */ E("svg", {
								viewBox: "0 0 16 16",
								width: "12",
								height: "12",
								fill: "none",
								stroke: "currentColor",
								strokeWidth: "1.6",
								strokeLinecap: "round",
								strokeLinejoin: "round",
								"aria-hidden": "true",
								className: y("transition-transform duration-150", w && "rotate-90"),
								children: /* @__PURE__ */ E("path", { d: "M6 4l4 4-4 4" })
							}), "Structured filters"]
						}), w && /* @__PURE__ */ E("div", {
							className: "mt-3",
							children: /* @__PURE__ */ E(J, {
								busy: g === "structured",
								onRun: (e) => void j(e)
							})
						})]
					})
				]
			}),
			N && /* @__PURE__ */ E(W, {
				plan: N,
				planner: F,
				nameFor: t,
				dryRun: p != null
			}),
			p && /* @__PURE__ */ D(f, {
				title: "Dry run — not executed",
				subtitle: "The plan above was generated but no query was run",
				children: [/* @__PURE__ */ E("p", {
					className: "text-xs leading-relaxed text-fg-secondary",
					children: "Nothing was read from the fact tables. Review the interpreted plan, then execute it exactly as shown."
				}), /* @__PURE__ */ D("div", {
					className: "mt-3 flex items-center gap-2",
					children: [/* @__PURE__ */ E(s, {
						variant: "primary",
						disabled: g !== null,
						onClick: () => void j(p.plan),
						children: g === "structured" ? /* @__PURE__ */ D(T, { children: [/* @__PURE__ */ E(h, { size: 14 }), "Running…"] }) : "Run this plan"
					}), /* @__PURE__ */ E(s, {
						disabled: g !== null,
						onClick: () => void k(n),
						children: "Re-run as search"
					})]
				})]
			}),
			a && /* @__PURE__ */ D(T, { children: [/* @__PURE__ */ E(q, {
				result: a,
				nameFor: t
			}), /* @__PURE__ */ E(ee, { proof: a.proof })] }),
			!S && !N && /* @__PURE__ */ E(c, {
				title: "Ask a question to begin",
				hint: "Search in plain language across entry, zone and breach events. The interpreted plan and a proof ladder are shown with every result, so you can always see how the question was read and why the answer holds."
			})
		]
	});
}
function re({ active: e, onClick: t, children: n }) {
	return /* @__PURE__ */ E("button", {
		type: "button",
		onClick: t,
		className: y("relative -mb-px whitespace-nowrap border-b-2 px-3.5 py-2.5 font-mono text-[11px] font-semibold uppercase tracking-micro transition-colors duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 focus-visible:ring-offset-canvas", e ? "border-accent text-fg" : "border-transparent text-fg-muted hover:text-fg-secondary"),
		children: n
	});
}
function Q() {
	let [r, a] = i(null), [c, l] = i(!0), [u, m] = i(!1), [g, _] = i(null), [y, b] = i("query"), x = e(async () => {
		l(!0), _(null);
		try {
			let e = await v.me();
			a(e), m(!1);
		} catch (e) {
			e instanceof o && e.status === 401 ? (a(null), m(!0)) : _(e instanceof Error ? e.message : String(e));
		} finally {
			l(!1);
		}
	}, []);
	t(() => {
		x();
	}, [x]);
	let S = w(() => v.listCameras(), 0), T = S.data ?? [], O = n(() => {
		let e = /* @__PURE__ */ new Map();
		for (let t of T) e.set(t.id, t.name);
		return e;
	}, [S.data]), k = e((e) => e ? O.get(e) ?? e : "—", [O]);
	async function A() {
		try {
			await v.logout();
		} catch {}
		C(null), a(null), m(!0);
	}
	return u ? /* @__PURE__ */ E(d, { onSuccess: (e) => {
		a(e), m(!1), _(null);
	} }) : c && !r ? /* @__PURE__ */ D("div", {
		className: "flex min-h-[60vh] items-center justify-center gap-3 text-fg-secondary",
		children: [/* @__PURE__ */ E(h, {}), /* @__PURE__ */ E("span", {
			className: "font-mono text-xs uppercase tracking-micro",
			children: "Authenticating…"
		})]
	}) : g && !r ? /* @__PURE__ */ E("div", {
		className: "mx-auto max-w-md px-4 py-20",
		children: /* @__PURE__ */ D(f, {
			title: "Console unavailable",
			children: [/* @__PURE__ */ E(L, { children: g }), /* @__PURE__ */ E("div", {
				className: "mt-3 flex justify-end",
				children: /* @__PURE__ */ E(s, {
					variant: "primary",
					onClick: () => void x(),
					children: "Retry"
				})
			})]
		})
	}) : r ? /* @__PURE__ */ D("div", {
		className: "mx-auto max-w-[1600px] px-4 py-6 sm:px-6",
		children: [/* @__PURE__ */ D("header", {
			className: "animate-rise",
			children: [/* @__PURE__ */ D("div", {
				className: "flex flex-wrap items-end justify-between gap-4",
				children: [/* @__PURE__ */ D("div", {
					className: "min-w-0",
					children: [/* @__PURE__ */ E(p, { children: "Intelligence · Search" }), /* @__PURE__ */ E("h1", {
						className: "mt-1 font-display text-2xl font-extrabold tracking-tight text-fg",
						children: "Semantic Search"
					})]
				}), /* @__PURE__ */ D("div", {
					className: "flex items-center gap-3",
					children: [/* @__PURE__ */ D("div", {
						className: "flex flex-col items-end leading-none",
						children: [/* @__PURE__ */ E("span", {
							className: "font-mono text-[12px] font-semibold text-fg",
							children: r.name
						}), /* @__PURE__ */ D("span", {
							className: "mt-1 font-mono text-[9px] uppercase tracking-micro text-accent",
							children: [r.role, r.kind === "system" && /* @__PURE__ */ E("span", {
								className: "text-fg-muted",
								children: " · auth off"
							})]
						})]
					}), r.kind === "user" && /* @__PURE__ */ E(s, {
						size: "sm",
						onClick: () => void A(),
						children: "Sign out"
					})]
				})]
			}), /* @__PURE__ */ E("div", {
				className: "mt-5 flex flex-wrap gap-1 overflow-x-auto border-b border-line",
				children: [{
					key: "query",
					label: "Query"
				}, {
					key: "semantic",
					label: "Semantic"
				}].map((e) => /* @__PURE__ */ E(re, {
					active: y === e.key,
					onClick: () => b(e.key),
					children: e.label
				}, e.key))
			})]
		}), /* @__PURE__ */ E("div", {
			className: "mt-5",
			children: y === "query" ? /* @__PURE__ */ E(Z, { nameFor: k }) : /* @__PURE__ */ E(X, {
				nameFor: k,
				cameras: T
			})
		})]
	}) : null;
}
//#endregion
//#region src/modules/search/entry.tsx
var $ = Q;
//#endregion
export { $ as default };
