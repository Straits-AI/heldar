...
/// Placeholder emitted in the `[Interface] PrivateKey` line when the CLIENT generated its own keypair
/// and only sent us its public key. The client (mobile app / dashboard) substitutes its locally-held
/// private key into the returned config. This is the SECURE enrollment path: the peer private key is
/// never generated by, sent to, or returned from the server.
const CLIENT_KEY_PLACEHOLDER: &str = "__REPLACE_WITH_DEVICE_PRIVATE_KEY__";

/// A WireGuard key is base64 of exactly 32 bytes (Curve25519). Used to validate a client-supplied
/// public key before it touches `wg`.
...
/// Build the split-tunnel client `.conf` (routes ONLY the Heldar host over the tunnel). `private_line`
/// is either the real `PrivateKey = …` (server-generated fallback) or the client-key placeholder line.
fn peer_config(
    private_line: &str,
    peer_ip: &Ipv4Addr,
    host_pub: &str,
...
pub fn add_peer(
    ...
    // SECURE path: the client generated its own keypair and sent only the public key — we never see a
    // private key. Fallback path (no client key): we generate the keypair and return it in the config
    // (convenient for curl, but the key crosses the wire — clients SHOULD send their own public key).
    let (peer_pub, private_line) = match client_public_key {
        Some(pk) => {
            let pk = pk.trim();
            anyhow::ensure!(
                is_valid_wg_key(pk),
                "client public_key is not a valid WireGuard key"
            );
            (
                pk.to_string(),
                format!("PrivateKey = {CLIENT_KEY_PLACEHOLDER}"),
            )
        }
        None => {
            let peer_priv = run("wg", &["genkey"])?.trim().to_string();
            let peer_pub = pubkey_of(&peer_priv)?;
            (peer_pub, format!("PrivateKey = {peer_priv}"))
        }
    };
    run(
        "wg",
...
mod tests {
    ...
    fn wg_key_validation() {
        // 32 zero bytes, base64 → a valid WireGuard key shape.
        let valid = base64::engine::general_purpose::STANDARD.encode([0u8; 32]);
        assert!(is_valid_wg_key(&valid));
        assert!(!is_valid_wg_key("not-base64!!"));
    ...
    #[test]
    fn client_gen_config_has_placeholder_not_a_key() {
        // The client-key path must NEVER embed a real private key — only the placeholder.
        let ip = Ipv4Addr::new(10, 200, 0, 2);
        let cfg = peer_config(
            &format!("PrivateKey = {CLIENT_KEY_PLACEHOLDER}"),
            &ip,
            "hostpubkey=",
