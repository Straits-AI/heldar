//! Heldar composing server — a LIBRARY plus a thin bin (`src/main.rs`).
//!
//! Links the open kernel with the open generic apps (access control, movement, search). Vertical
//! composition is a first-class seam: [`run`] takes an implementation of the [`Verticals`] trait,
//! so any workspace — this one, or a private overlay repo — can produce its own composed binary
//! without forking this crate (`heldar_server::run(MyVerticals)` is a complete server).
//!
...
/// The vertical-composition seam. Implement this to link proprietary (or third-party) vertical
/// apps into the composed server; every method defaults to a no-op, so `run(NoVerticals)` is the
/// complete open reference server. All methods are called exactly once during boot, at the same
/// points the open apps compose.
///
/// Native async-fn-in-trait (no boxing): [`run`] takes `impl Verticals` — static dispatch, awaited
/// once from the binary's entry future, so no `Send` bound on the returned future is needed.
#[allow(async_fn_in_trait)]
pub trait Verticals: Send + Sync {
    /// Module manifests to append to `GET /api/v1/modules` (drives the dashboard nav).
    fn manifests(&self) -> Vec<heldar_kernel::modules::ModuleManifest> {
        Vec::new()
    }
    /// Apply vertical schemas (idempotent) against the shared pool, after the open apps'.
    /// Takes the pool by value (a cheap handle clone) — a borrowed pool across the boxed
    /// async-trait future trips sqlx's higher-ranked `Executor` bounds.
    async fn init_schema(&self, _pool: sqlx::SqlitePool) -> anyhow::Result<()> {
        Ok(())
    }
...
/// The open reference composition: kernel + generic apps, no verticals.
pub struct NoVerticals;
impl Verticals for NoVerticals {}

/// Boot and serve the composed server. This IS the product: config, migrations, schemas,
/// consumers, background services, routers, and the HTTP listener — parameterized only by the
/// vertical composition. One-shot admin subcommands (argv[1]) run and exit inside here too.
pub async fn run(verticals: impl Verticals) -> anyhow::Result<()> {
    dotenvy::dotenv().ok();
    init_tracing();
    ...
    // Supervise the background services: if one panics, it is respawned (production resilience).
    {
        ...
        // Box-side WebRTC rendezvous: dial OUT to the public signaling service for universal remote
        // viewing (ADR 0003, P2). Bridges browser WHEP sessions to the local MediaMTX. Parks unless
        // HELDAR_REMOTE_RENDEZVOUS_URL + HELDAR_SITE_ID are set (remote access is opt-in).
        {
            let st = state.clone();
            spawn_supervised("webrtc_rendezvous", move || {
...
/// Public so vertical compositions supervise their own loops the same way.
