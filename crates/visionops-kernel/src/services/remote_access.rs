...
//!
//! A VisionOps deployment is typically behind CGNAT (no inbound port-forward, DDNS useless). The
...
pub fn status(cfg: &Config) -> OverlayStatus {
    if !cfg.overlay_enabled {
        return OverlayStatus {
            ...
            operstate: None,
            up: false,
            note: "Remote-access overlay disabled (VISIONOPS_OVERLAY_ENABLED=false); reachable on the LAN only."
                .into(),
        };
    }
    let Some(iface) = cfg.overlay_iface.clone() else {
        return OverlayStatus {
            ...
            operstate: None,
            up: false,
            note: "Overlay enabled but VISIONOPS_OVERLAY_IFACE is unset; cannot determine status."
                .into(),
        };
    };
