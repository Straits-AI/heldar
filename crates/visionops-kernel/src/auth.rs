...
pub async fn ensure_bootstrap(pool: &SqlitePool, cfg: &Config) -> anyhow::Result<()> {
    ...
    match (&cfg.bootstrap_admin_user, &cfg.bootstrap_admin_password) {
        ...
        (Some(_), Some(_)) => {
            tracing::error!(
                "auth: VISIONOPS_BOOTSTRAP_ADMIN_PASSWORD must be >= 8 chars; no admin created"
            );
        }
        _ => {
            tracing::warn!(
                "auth: enabled but no users exist and VISIONOPS_BOOTSTRAP_ADMIN_USER/PASSWORD not set; \
